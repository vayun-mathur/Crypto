...
@Composable
private fun StockActionCard(viewModel: PortfolioViewModel, usdcToken: TokenInfo, stockToken: TokenInfo) {
    var isDeposit by remember { mutableStateOf(true) }
    var amount by remember { mutableStateOf("") }

    val fromToken = if (isDeposit) usdcToken else stockToken
    val toToken = if (isDeposit) stockToken else usdcToken
    ...
    LaunchedEffect(amount, isDeposit) {
        ...
        if (amountDouble > 0) {
            val fromAmountForApi = if (isDeposit) {
                ...
                val stockTokenPrice = TokenPriceRepository[stockToken]?.price ?: 0.0
                val usdcTokenPrice = TokenPriceRepository[usdcToken]?.price ?: 0.0
                if (usdcTokenPrice > 0) {
                    amountDouble * (usdcTokenPrice / stockTokenPrice)
                } else {
                    0.0
                }
            }
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            OutlinedTextField(
                ...
                label = { Text("Amount in ${usdcToken.symbol}") },
